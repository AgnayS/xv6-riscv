...
{
  struct proc *p = myproc();
  struct proc *tp; // our new thread proc
  int havekids, pid;
  ...
  for (;;)
  {
    // initially, no child threads
    havekids = 0;

    for (tp = proc; tp < &proc[NPROC]; tp++)
    {
      if (tp->parent == p && tp->isParentThread == 0)
      { // Check if tp is a child thread of p.
        ...
        if (tp->state == ZOMBIE)
        {
          ...
          // Exit status collection jargon
          if (addr != 0 && copyout(p->pagetable, addr, (char *)&tp->xstate, sizeof(tp->xstate)) < 0)
          {
            release(&tp->lock);
            release(&wait_lock);
            return -1;
